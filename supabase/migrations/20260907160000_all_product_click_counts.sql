  ...
  select json_build_object(
    ...
    'products', coalesce((select json_agg(row_to_json(product_stats) order by product_stats.clicks desc, product_stats.name)
      from (select p.id, p.name, count(pc.id)::integer as clicks
            ...
            group by p.id, p.name order by count(pc.id) desc) product_stats), '[]'::json),
    ...
    'recent_clicks', coalesce((select json_agg(row_to_json(recent_click) order by recent_click.clicked_at desc)
      from (select pc.product_id, p.name, pc.clicked_at from public.product_clicks pc join public.products p on p.id = pc.product_id order by pc.clicked_at desc limit 50) recent_click), '[]'::json),
    'searches', coalesce((select json_agg(row_to_json(search_stats) order by search_stats.searches desc, search_stats.search_term)
      from (select search_term, count(*)::integer as searches from public.search_queries where searched_at >= now() - interval '30 days' group by search_term order by count(*) desc, search_term limit 20) search_stats), '[]'::json),
